{ pkgs, mode ? "stable", ... }: {
  ...
  bootstrap = ''
    ...
    npm pkg set scripts.compile="npx tsc && npx tsc -p client/logic && npx dep fix-paths && npx dep fix-paths -p client/logic"
    ...
    npm pkg set scripts.fix="npx dep fix-paths --watch"
    npm pkg set scripts.fix-client="npx dep fix-paths -p client/logic --watch"
